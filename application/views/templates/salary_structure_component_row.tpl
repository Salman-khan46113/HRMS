...
{if count($component_details) > 0}
	...
	<tr class="row_{{$key_val}} {{if $component_val['used'] eq 'Yes' && $form_type eq 'salary_structure'}}used_component{{/if}} component_row">
	    ...
	    <td class="text-center">
	    	<input type="text" class="form-control number_box number-formatter" id="component_value_{{$key_val}}" name="component_value[{{$key_val}}]"  placeholder="Enter Component Value" value="{{getNumberFormate($component_val['component_value'])}}">
	    </td>
	    <td class="text-center">
